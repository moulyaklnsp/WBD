...
const TournamentsService = {
  ...
  async updateTournament(db, user, { id, body }) {
    ...
    if (date) {
      ...
      if (!isSameDate) {
        if (incomingDate.getTime() < existingDate.getTime()) {
          throw createError('You cannot prepone (move to an earlier date) the tournament.', 400);
        }
        const diffDays = (incomingDate.getTime() - existingDate.getTime()) / (1000 * 3600 * 24);
        if (diffDays > 3) {
          throw createError('You can only postpone the tournament by a maximum of 3 days.', 400);
        }
      }
      $set.date = parsedDate;
    }
    ...
    if (status === 'approved' || status === 'ongoing') {
      await Cache.invalidateTags(['tournaments'], { reason: 'coordinator.tournaments.update' });
    }
  ...
  async deleteTournament(db, user, { id }) {
    ...
    if (result.modifiedCount > 0) {
      const status = String(existing.status || '').toLowerCase();
      if (status === 'approved' || status === 'ongoing') {
        await Cache.invalidateTags(['tournaments'], { reason: 'coordinator.tournaments.delete' });
      }

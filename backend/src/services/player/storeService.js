...
const StoreService = {
  ...
  async buyProduct(db, user, body) {
    ...
    // Sales + product availability changed - invalidate store caches (suggestions, most ordered, etc).
    await Cache.invalidateTags(['store'], { reason: 'store.buyProduct' });
  ...
  async getStoreSuggestions(db, user) {
    ...
    const { value: mostOrdered } = await Cache.cacheAsideJson({
      key: Cache.keys.storeMostOrdered(),
      ttlSeconds: Cache.config.ttl.defaultSeconds,
      tags: ['store'],
      label: 'Store.getStoreSuggestions mostOrdered',
      fetcher: async () => SalesModel.aggregate(database, [

...
const IS_TEST = process.env.NODE_ENV === 'test' || process.env.JEST_WORKER_ID != null;
...
let backendAccessStream = null;
let backendErrorStream = null;
...
if (!IS_TEST) {
  ...
  backendAccessStream = rfs.createStream('access.log', {
  interval: '1d',
  maxFiles: 14,
  path: backendLogsDir,
  compress: 'gzip'
  ...
  backendErrorStream = rfs.createStream('error.log', {
  interval: '1d',
  maxFiles: 14,
  path: backendLogsDir,
  compress: 'gzip'
...
// ─── Frontend log stream ──────────────────────────────────────────────────────
  ...
  frontendLogStream = rfs.createStream('frontend.log', {
  interval: '1d',
  maxFiles: 14,
...
if ((process.env.NODE_ENV || 'development') !== 'production' && !IS_TEST) {

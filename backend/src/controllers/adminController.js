  ...
  try {
    ...
    const enrichedCoordinators = await Promise.all(coordinators.map(async (coord) => {
      const identityRegexes = [
        new RegExp(`^${(coord.name || '').replace(/[-/\\^$*+?.()|[\]{}]/g, '\\const getCoordinators = async (req, res) => {
  try {
    const db = await connectDB();
    const coordinators = await db.collection('users')
      .find({ role: 'coordinator' })
    ...
    res.json(coordinators);')}const { connectDB } = require('../config/database');
const moment = require('moment');
const helpers = require('../utils/helpers');
const path = require('path');
const { sendContactStatusEmail } = require('../services/emailService');
...
// Coordinators API
, 'i'),
        new RegExp(`^${(coord.email || '').replace(/[-/\\^$*+?.()|[\]{}]/g, '\\const getCoordinators = async (req, res) => {
  try {
    const db = await connectDB();
    const coordinators = await db.collection('users')
      .find({ role: 'coordinator' })
...
const getOrganizers = async (req, res) => {
  try {
    ...
    const enrichedOrganizers = await Promise.all(organizers.map(async (org) => {
      const identityRegexes = [

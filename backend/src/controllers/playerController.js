...
const getStore = async (req, res) => {
  try {
    const user = getSessionUser(req, res);
    if (!user) return;
    const data = await StoreService.getStore(null, user);
    return res.status(200).json(data);
  } catch (error) {
...
const getStoreSuggestions = async (req, res) => {
  try {
    const user = getSessionUser(req, res);
    if (!user) return;
    const data = await StoreService.getStoreSuggestions(null, user);
    return res.status(200).json(data);
  } catch (error) {
...
const getPlayerStreams = async (req, res) => {
  try {
    const user = getSessionUser(req, res);
    if (!user) return;
    const data = await StreamsService.getPlayerStreams(null, user);
    return res.status(200).json(data);
  } catch (error) {
...
const getAnnouncements = async (req, res) => {
  try {
    const user = getSessionUser(req, res);
    if (!user) return;
    const data = await NotificationsService.getAnnouncements(null);
    return res.status(200).json(data);
  } catch (error) {

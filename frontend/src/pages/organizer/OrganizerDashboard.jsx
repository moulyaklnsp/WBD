...
function OrganizerDashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      setPendingCoordinators(Array.isArray(data.pendingCoordinators) ? data.pendingCoordinators : []);
    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <div className="page player-neo">
        ...
        <div className="content">
          ...
          >
            <h3><i className="fas fa-users-cog" /> Coordinator Approvals</h3>
            <ul style={{ padding: 0 }}>
              {loading ? (
                <li>Loading coordinators...</li>
              ) : error ? (
                <li style={{ color: 'crimson' }}>{error}</li>
              ) : pendingCoordinators.length === 0 ? (
                <li>No pending coordinator requests.</li>
              ) : (
                pendingCoordinators.map((c, idx) => (
                  <li key={c._id || idx}>
                    <i className="fas fa-user-circle" style={{ fontSize: '1.2rem', color: 'var(--sea-green)' }} />
                    <div className="meeting-info" style={{ marginLeft: '1rem' }}>
                      <strong>{c.data?.name || c.email}</strong>
                      <div className="date-tag">
                        Email: {c.email} {c.data?.college ? `• College: ${c.data.college}` : ''}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button className="more-btn" onClick={() => handleCoordinatorApproval(c.email, true)}>Approve</button>
                      <button className="more-btn" style={{ background: 'var(--accent-red)' }} onClick={() => handleCoordinatorApproval(c.email, false)}>Reject</button>
                    </div>
                  </li>
                ))
              )}

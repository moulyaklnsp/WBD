...
function resolveBlogImages(blog) {
  ...
  return combined.map((raw) => normalizeImageSrc(raw));
}
...
export default function CoordinatorBlogDetail() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const loadBlog = async () => {
      try {
        setLoading(true);
        setError('');
        const res = await fetchAsCoordinator(`/coordinator/api/blogs/${id}`);
    ...
    <div className="page player-neo" style={{ minHeight: '100vh' }}>
      ...
      <main className="content">
        <GlassCard>
          ...
          >
            <Link to="/coordinator/blogs" style={{ color: '#87CEEB', textDecoration: 'none', fontFamily: "'Cinzel', serif" }}>
              <i className="fas fa-arrow-left" /> Back to Blogs
            </Link>
          </motion.div>

          {loading ? (
            <p style={{ textAlign: 'center', color: 'rgba(255, 253, 208, 0.85)' }}>Loading blog...</p>
          ) : error ? (

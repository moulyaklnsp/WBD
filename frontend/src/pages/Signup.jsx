...
export default function Signup() {
  ...
  useEffect(() => {
    if (auth.pendingApproval && form.email) {
      let isMounted = true;
      const initSocket = async () => {
        ...
        if (loaded && window.io && isMounted) {
          ...
          socketRef.current.on('connect', () => {
            socketRef.current.emit('join_signup', { email: form.email });
          });

          socketRef.current.on('coordinator_approved', () => {
            dispatch(setCoordinatorApproved());
            setServerMsg({ type: "success", text: "Approved! OTP sent to your email." });
          });

          socketRef.current.on('coordinator_rejected', () => {
            dispatch(setCoordinatorRejected());
            setServerMsg({ type: "error", text: "Signup request was rejected by an organizer." });
          });
        }
      };
      initSocket();
  ...
  }, [auth.pendingApproval, form.email, dispatch]);
  ...
  function validateRole(r) { return ['organizer', 'coordinator', 'player'].includes(r); }
    ...
    try {
      ...
      if (result.meta.requestStatus === 'fulfilled') {
        if (result.payload?.pendingApproval) {
          setServerMsg({ type: "success", text: result.payload.message || "Signup request sent for approval. Please wait..." });
        } else {
          ...
          >
            <GlassCard delay={0.3}>
              ...
              {auth.pendingApproval ? (
                ...
                >
                  <GlobalLoader style={{ marginBottom: '1rem' }} />
                  <h3 style={{ color: '#FFFDD0', fontFamily: "'Cinzel', serif" }}>Waiting for Organizer Approval...</h3>
                  <p style={{ color: 'rgba(255, 253, 208, 0.7)', fontSize: '0.9rem', marginTop: '0.5rem' }}>
                    Your request has been sent to organizers. Please keep this tab open to automatically receive your OTP upon approval.
                  </p>
                </motion.div>
              ) : (
